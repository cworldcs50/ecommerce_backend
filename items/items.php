...
$query = "SELECT items_view.*, 1 AS is_favorite
    ...
    INNER JOIN users 
    ON favorites.favorites_users_id = ? 
    WHERE items_view.categories_id = ?

UNION ALL

SELECT items_view.*, 0 AS is_favorite 
FROM items_view
    WHERE items_view.categories_id = ? AND items_view.items_id 
    NOT IN ( SELECT items_view.items_id 
    FROM items_view 
    INNER JOIN favorites
    ON favorites.favorites_items_id = items_view.items_id
    INNER JOIN users 
